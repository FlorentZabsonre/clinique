...
if ( isset ( $_POST['envoi3'] )) {
	...
	if (mysqli_num_rows($result)!=0){
 		while($row = mysqli_fetch_row($result)){
			...
			$mdp = $row[3];
			$rang = $row[4];
 			echo '<p><label for="b"><input type="checkbox" name="EmployésASuppr[]" value="'.$id.'">Client numéro '.$id.':</label><input type="text" readonly="readonly" value="'.$nom.' '.$prenom.' '.$mdp.' '.$rang.'"size="30" /></p>' ;
		}
